<!DOCTYPE html>
<html lang="en-us">

<head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
    <title>LiterallyGames Home</title>
    <link rel="stylesheet" type="text/css" href="index.css">
    <link href="https://fonts.googleapis.com/css2?family=Indie+Flower&display=swap" rel="stylesheet">
    <link href="favicon.ico" rel="shortcut icon" type="image/x-icon" />

    <script src="index.js"></script>
</head>

<body id="main">
    <header>
        <nav>
            <ul class="navs">
                <li id="teamname" onclick="Prankfunction()">LiterallyGames</li>
                <li><a href="#">Home</a></li>
                <li><a id="about" onmouseover="prankfunction()" onmouseout="prankrestored()" href="http://www.innersloth.com/gameAmongUs.php">About Us</a><img id="cyan" src="cyan.png"></img></li>
          </ul>
        <div class="dropdown">
            <button class="dropbtn"><img id="navimg" src="unknow.jpg"></button>
            <div class="dropdown-content">
                <a class = "settings" href="#"><span id="usernamehere">username</span></a>
                <a class = "settings" href="UserSett.php">Reset Password</a>
                <a class = "settings" href="sign_out.php">Logout</a>
            </div>
        </div>
        </nav>

        
    </header>

    <div class="rainbow-text">
        <p class="teamslogan">To game, <br>or not to game.<br> Saviors shall help you.</p>
    </div>
    <button class="button"><a href="sign_up.php">Join NOW!</a></button>
    <div class="buttom_1">
        <div class="wave wave_1"></div>
    </div>
    <div class="buttom_2">
        <div class="wave wave_2"></div>
    </div>
    <div class="buttom_3">
        <div class="wave wave_3"></div>
    </div>

</body>

</html>